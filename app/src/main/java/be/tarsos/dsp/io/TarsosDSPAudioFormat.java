...
public class TarsosDSPAudioFormat {
     ...
     * このフォーマットで使用されるオーディオエンコーディングの技術
     */
    protected Encoding encoding;

    /**
     * The number of samples played or recorded per second, for sounds that have this format.
     * 秒間あたりに、このフォーマットがもつ 再生時もしくは録音されるサンプルの数です。
     */
    protected float sampleRate;

    /**
     * The number of bits in each sample of a sound that has this format.
     * サウンドの各サンプルに含まれるビット数です
     */
    protected int sampleSizeInBits;

    /**
     * The number of audio channels in this format (1 for mono, 2 for stereo).
     * オーディオチャンネル数を示す（1ならばモノラル・２ならばステレオ）
     */
     ...
     * オーディオデータをメモリに格納する際の書き込み方式を、Big-Endian（データの桁の大きい方から格納する）とするか
     * Little-endian（データの桁の小さい方から格納するか）をboolean型で指定する。TrueならBig-Endian。
     */
    ...
    /**キーの型名がString、値の型名がObject型、宣言のみで初期化はされていない。
     * 値はObject型なので、「どんな型」でも入るようになっている
     * */
